...
def agregar_profile(username, F_nombre, S_nombre, P_Apellido, S_Apellido, Sexo, Nacimiento):
    dato = UserProfile()
    dato.user = User.objects.get(username=username)
    dato.image = None
    dato.ninho = Ninho.objects.get(P_Nombre=F_nombre, S_Nombre=S_nombre, Apellido_P=P_Apellido, Apellido_M=S_Apellido, sexo=Sexo, fecha_nacimiento=Nacimiento)
    dato.save()