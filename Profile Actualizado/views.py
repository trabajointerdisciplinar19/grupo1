...
def registro(request):
    if request.method == 'POST':
        ...
        agregar_profile(username,F_nombre,S_nombre,P_Apellido,S_Apellido,Sexo,Nacimiento)
...
def perfil(request):
    if request.user.is_authenticated:
        ...
        my_request_id = request.user.id
        myboy = UserProfile.objects.filter(user_id=request.user.id)
        myid=myboy.get(user_id=request.user.id)
        print(myid.ninho.P_Nombre)
        values = {'user': request.user,
                  'ninho': myid.ninho}
